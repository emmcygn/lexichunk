"""Heading shapes: the ones that must be found, and the ones that must not.

Companion to ``tests/test_heading_regression.py``, which pins the
plausibility gate's own realistic-heading and numeric-prose tables. This
module pins the per-line ``detect_level`` shapes underneath it, US
bare-decimal headings in particular.
